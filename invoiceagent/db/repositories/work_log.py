...
from invoiceagent.db.models import WorkLog, Project
...
class WorkLogRepository(BaseRepository[WorkLog]):
    ...
    def get_by_date_range(self, session: Session, start_date: date, end_date: date) -> List[WorkLog]:
    ...
    def get_by_client_id(self, session: Session, client_id: int) -> List[WorkLog]:
        ...
        """
        return session.query(WorkLog).join(
            WorkLog.project
        ).filter(
            WorkLog.project.has(client_id=client_id)
        ).order_by(WorkLog.work_date).all()
    ...
    def get_unbilled(self, session: Session) -> List[WorkLog]:
        ...
        return session.query(WorkLog).filter(
            ...
            WorkLog.invoice_id == None
        ).order_by(WorkLog.work_date).all()

    def get_total_hours_by_project(self, session: Session, start_date: date, end_date: date) -> List[tuple]:
    ...
    def get_with_project_and_client(self, session: Session, work_log_id: int) -> Optional[WorkLog]:
        ...
        """
        return session.query(WorkLog).options(
            joinedload(WorkLog.project).joinedload(Project.client)
